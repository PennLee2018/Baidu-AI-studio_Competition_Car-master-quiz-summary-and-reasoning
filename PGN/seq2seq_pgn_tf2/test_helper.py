...
def greedy_decode(model, dataset, vocab, params):
    ...
    '''
    一个batch的内部结构：
    batch: ({'enc_input': <tf.Tensor: shape=(10, 115), dtype=int32, numpy=
    array([[  37,    5,  764, ...,    6,    7,  768],
       [ 394,   33,  114, ...,    1,    1,    1],
       [ 764, 1429,  184, ...,    1,    1,    1],
       ...,
       [1471, 1472,   90, ...,    1,    1,    1],
       [  54,   64,   35, ...,    1,    1,    1],
       [1488,  790,   79, ...,    1,    1,    1]])>, 
       'extended_enc_input', 'article_oovs', 'max_oov_len', 'sample_encoder_pad_mask'},
       {'dec_input', 'dec_target', 'dec_len' })
    '''
    
    # for batch in tqdm(dataset):
    #     #print("batch:",batch)
    #     results += batch_greedy_decode(model, batch, vocab, params)
    
    dataset_iter=iter(dataset)
    for i in tqdm(range(steps_epoch)):
        enc_data, _ = next(dataset_iter)#数据一批批送入
        #print("\n enc_data\n",enc_data)
...
def beam_decode_default(model, dataset, vocab, params):
    # 存储结果
    batch_size = params["batch_size"]
    results = []
    sample_size = TEST_NUM_SAMPLES #20000(小样本数50)
    ...
    for batch in tqdm(dataset):
        #print("batch:",batch)
        # abstract = batch_beam_decode2(model, batch, vocab, params)
        # results.append(abstract)
        results += batch_beam_decode2(model, batch, vocab, params)
    return results
